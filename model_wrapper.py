...
class ModelWrapper:
    ...
    def __init_train__(self, args, model, num_train_steps, logger, is_program_as_sequence):
        ...
        print("+++>", type(self.args.reload_model_path))
        if self.args.reload_model_path != "nothing":
            print("--->self.args.reload_model_path是：")
            print("--->", self.args.reload_model_path)
        ...
        logger.info("after restore_from_prev")
    def __init_test__(self, args, model):
        ...
        self.model.load_state_dict(torch.load(self.args.reload_model_path),False)
        if self.args.cuda:
            self.model.cuda()
    ...
    def evaluate(self, batch, op_list, const_list, require_loss):
        self.model.eval()
        print("ModelWrapper.evaluate()运行")
        print("ModelWrapper.evaluate() batch")
        print(batch)
        # get the batch inputs for the model
        input_ids = batch["input_ids"]
        input_mask = batch["input_mask"]
    ...
    def evaluate_finqa_baseline(self, batch, op_list, const_list):
        self.model.eval()
        # is_training, input_ids, input_mask, segment_ids, program_ids, option_mask
        print("ModelWrapper.evaluate_finqa_baseline()运行");
        print("ModelWrapper.evaluate_finqa_baseline() batch")
        print(batch)
        input_ids = batch["input_ids"]
        input_mask = batch["input_mask"]
        segment_ids = batch["segment_ids"]
